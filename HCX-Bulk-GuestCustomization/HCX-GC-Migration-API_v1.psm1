Function Connect-HcxServerAPI {
    ...
    #>
        ...
        Param (
            ...
            [Parameter(Mandatory=$false)][bool]$SeedCheckpoint=$false,
            ...
            [Parameter(Mandatory=$false)][bool]$ReplicateSecurityTags=$false,
            [Parameter(Mandatory=$false)][bool]$MigrateCustomAttributes=$false,
        ...
        If (-Not $global:hcxConnection) { Write-error "HCX Auth Token not found, please run Connect-HcxServerAPI " } Else {
            ...
            $input = @{
                    ...
                    "switchoverParams" = @{
                        ...
                        "options" =@{
                            "forcePowerOffVm"=$ForcePowerOffVm;
                            "upgradeVMTools"=$UpgradeVMTools;
                            "upgradeHardware"=$UpgradeHardware;
                            "replicateSecurityTags"=$ReplicateSecurityTags;
                            "updateCustomAttributes"=$MigrateCustomAttributes;
                            "retainMac"=$RetainMac;
                        }
                        "schedule" =@{
                            "startTime"=$StartTime;
            ...
            if ($GuestCustomization) {
                ...
                $PrimaryDnsServers = @() + $GCNetCustomization[0].DnsServers
                $PrimaryDnsSuffix = @($GCNetCustomization[0].DnsSuffix)

                $guestCustom = @{
                    "changeSID"=$ChangeSID;
                    "networkCustomizations"=$netCustomArray;
                    "dnsCustomizations"=@{
                        "dns"=$PrimaryDnsServers;
                        "dnsSuffixes"=$PrimaryDnsSuffix;
                    }                    
                    "identity"=@{
                        "name"=$GCName;
                        "domainName"=$GCDomain;
            ...
            if($PSVersionTable.PSEdition -eq "Core") {
                try{
                    ...
                    Write-Verbose -Message "catch [System.Management.Automation.CmdletInvocationException]:`n$_.ErrorDetails.Message"
                }
                catch [System.Net.WebException] {   
                    $respStream = $_.Exception.Response.GetResponseStream()
                    $reader = New-Object System.IO.StreamReader($respStream)
                    $respBody = $reader.ReadToEnd() | ConvertFrom-Json
                    Write-Verbose -Message "catch [System.Net.WebException]:`n$reader.ReadToEnd()"
                }
            } else {
                try{
